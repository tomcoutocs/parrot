...
import { AnalyticsReports } from './tabs/analytics-reports'
import { AnalyticsVisualizations } from './tabs/analytics-visualizations'
...
export function AnalyticsLayout({ activeTab, onTabChange }: AnalyticsLayoutProps) {
  ...
  const hasTabPermission = (tabId: string): boolean => {
    ...
    const permissionKey = `analytics:${tabId}`
    
    // Check for new format (app:tab) or old format (just app name)
    return tabPermissions.includes(permissionKey) || tabPermissions.includes('analytics')
  }
  ...
  const getTabDisplayName = () => currentTab?.label ?? 'Analytics'
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'explorer':
        return <AnalyticsDataExplorer />
      case 'saved':
        return <AnalyticsSavedReports />
      case 'settings':
