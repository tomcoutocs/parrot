-- Lead Customization Settings Table (Space-Based)
-- Dedicated table for lead generation app customization settings
-- Settings are space-level (admin-controlled), not user-level

CREATE TABLE IF NOT EXISTS lead_customization_settings (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  space_id UUID NOT NULL, -- Space/company ID (admin-controlled)
