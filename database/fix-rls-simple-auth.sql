-- Simple RLS Fix using Standard Supabase Auth
-- This script creates RLS policies that work with standard Supabase authentication
...
-- Step 4: Create simple policies using standard Supabase auth
SELECT '=== CREATING SIMPLE AUTH POLICIES ===' as info;

-- Policy 1: Allow all operations for authenticated users (simple approach)
CREATE POLICY "Allow authenticated users all operations" ON document_folders
  FOR ALL USING (auth.role() = 'authenticated');

-- Policy 2: Allow all operations for authenticated users on documents
...
BEGIN
  ...
  IF test_company_id IS NOT NULL AND test_user_id IS NOT NULL THEN
    ...
    BEGIN
      ...
      VALUES ('Simple Auth Test Folder', test_company_id, '/Simple Auth Test Folder', test_user_id)
...
-- Final status
SELECT '=== SIMPLE AUTH RLS FIX COMPLETE ===' as status;
SELECT 'Folder creation should now work with standard Supabase authentication.' as message;
