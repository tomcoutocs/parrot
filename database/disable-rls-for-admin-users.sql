-- Disable RLS for Admin Users - Comprehensive Script
-- This script modifies all RLS policies to allow admin users full access
...
-- Step 2: Check current policies for all tables
...
CREATE POLICY "Allow all authenticated users to see users" ON users
FOR ALL USING (
...
-- Step 8: Final status
SELECT '=== RLS DISABLE FOR ADMIN USERS COMPLETE ===' as status;
SELECT 'Admin users should now have full access to all tables.' as message;
SELECT 'Other users will still be restricted by company-based policies.' as note;
